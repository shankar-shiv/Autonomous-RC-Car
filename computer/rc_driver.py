            ...
            while True:
                ...
                if first != -1 and last != -1:
                    ...
                    if v_param1 > 0 or v_param2 > 0:
                        ...
                        self.d_stop_sign = d1  # stop before the stop sign
                        self.d_light = d2  # stop before traffic lights
                    ...
                    image_array = roi.reshape(
                        1, int(height/2) * width).astype(np.float32)
                    ...
                    if sensor_data and int(sensor_data) < self.d_sensor_thresh:
                        ...
                        self.stop_time = (
                            self.stop_finish - self.stop_start) / cv2.getTickFrequency()
                        ...
                        if stop_sign_active is False:
                            self.drive_time_after_stop = (
                                self.stop_start - self.stop_finish) / cv2.getTickFrequency()
